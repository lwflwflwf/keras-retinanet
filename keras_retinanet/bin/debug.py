...
# Allow relative imports when being executed as script. 当作为脚本执行时，允许相对导入。
if __name__ == "__main__" and __package__ is None:
    sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..'))  # .../bin/../.. ---> .../keras-retinanet
...
def create_generator(args):
    ...
    """
    # create random transform generator for augmenting training data为增加训练数据创建随机转换生成器
...
def parse_args(args):
    ...
    parser.add_argument('--annotations',
                        help='Show annotations on the image. Green annotations have anchors, red annotations don\'t and therefore don\'t contribute to training.',
                        action='store_true')
...
def run(generator, args, anchor_params):
    ...
    for i in range(generator.size()):
        ...
        # apply random transformations  支持随机转换
