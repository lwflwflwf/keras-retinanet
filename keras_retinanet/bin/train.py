...
def get_session():
    """ Construct a modified tf session.构造修改后的tf会话。
    """
...
def check_args(parsed_args):
    ...
    if parsed_args.multi_gpu > 1 and not parsed_args.multi_gpu_force:
        # 多GPU支持是实验性的  自担风险  想继续通过使用--multi-gpu-force来运行
        raise ValueError(
...
def parse_args(args):
    ...
    parser.add_argument('--snapshot-path',
                        ...
                        default='./snapshots')  # 训练过程中保存快照的路径
...
def main(args=None):
    ...
    # optionally load config parameters 可选加载配置参数
    if args.config:
